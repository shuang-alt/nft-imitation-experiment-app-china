...
function buildStudy1Pages(condition: Condition): ResolvedStudyPage[] {
  return [
    ...
    },
    {
      ...
      introLines: [
        "以下问题是关于 tiny dinos (eth) collection。",
        "请根据你的真实感受回答。",
      ],
    ...
    },
    {
      ...
      introLines: ["以下问题是关于之前看到的 NFT collection 与 tiny dinos (eth) 的关系。"],
      scaleLabel: likertScaleLabel,
      items: [
        "我觉得之前看到的 NFT collection 与 tiny dinos (eth) 风格相似",
        "之前看到的 NFT collection 让我联想到 tiny dinos (eth)",
        "根据页面信息，tiny dinos (eth) 似乎比之前看到的 collection 更早出现",
        "之前看到的 NFT collection 可以作为 tiny dinos (eth) 的替代",
        "我可能会购买之前看到的 NFT collection 而不是 tiny dinos (eth)",
        "之前看到的 NFT collection 降低了我对 tiny dinos (eth) 的购买兴趣",
      ],
      answerKeys: [
        "relationship_1",
        "relationship_2",
